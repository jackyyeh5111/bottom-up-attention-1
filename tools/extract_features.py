...
def load_image_ids(split_name):
    ...
    if split_name == 'coco_test2014':
      ...
      with open('/2t/jackyyeh/dataset/VisualGenome/image_data.json') as f:
        for item in json.load(f)[:10]:
          image_id = int(item['image_id'])
          # filepath = os.path.join('/2t/jackyyeh/dataset/VG/', item['url'].split('rak248/')[-1])
          filepath = os.path.join('/2t/jackyyeh/dataset/VG/', str(image_id) + '.jpg')
          split.append((filepath,image_id))  

    elif split_name == 'im2p':
      image_ids = []
      modes = ['train', 'val', 'test']
      # for mode in mode:
      mode = 'test' 
      with open('/2t/jackyyeh/im2p/data/image_ids/%s_imgs_ids.json' % mode, 'r') as f:
        t_ids = json.load(f)
        image_ids.extend(t_ids)
...
def merge_tsvs():
    n_gpu = 2
    test = ['./test_genome.tsv.%d' % i for i in range(n_gpu)]

    outfile = './bottom-up_test.tsv'
    with open(outfile, 'ab') as tsvfile:
        writer = csv.DictWriter(tsvfile, delimiter = '\t', fieldnames = FIELDNAMES)   
...
if __name__ == '__main__':
    ...
    image_ids = [image_ids[i::len(gpus)] for i in range(len(gpus))] # jacky deleted
